...
| lk:pin | pin | false | Use when the user explicitly asks to pin or link external authority or external content into the current project with provenance, version anchor or unlocked marker, update path, verify path, and script-backed materialization. Not for one-off web reading, package version pin only, UI/thread pinning, projection distribution, or copying content without a link script. | `skills/link/pin/SKILL.md` |
