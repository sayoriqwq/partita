...
- MUST 按轮次询问当前 frontier；每轮问题数量没有数值上限，也不得为了固定数量制造问题。
- MUST 让用户通过回答自然决定展开深度；不得预判或强制用户的深水区。
...
- 当前 Aim 不再成立时，MUST 显示 `Aim shift: <current> → <proposed>`；只有用户将该 shift 对齐后才能换 root，并按新 Aim 重算 tree。
...
- Conversation: MAY 显示 Aim、material epistemic lanes、frontier rounds、direct recommendations、Aim shift、Standard、closing receipt 和 persistence status。
...
1. 显示 `🧭 Align`，解析 active Aim；没有 Aim 时提议最小 Aim，并将其作为第一个 decision。
2. 读取 current conversation 与必要 evidence，把可查事实从 user decisions 中分离；自行解决事实 prerequisite。
3. 建立以 Aim 为 root、以 prerequisite 为 edge 的最小 internal decision tree；node 只标记 `Open` 或 `Aligned`，不输出 raw ledger。
4. 扫描 `Known known`、`Known unknown`、`Unknown known` 与 `Unknown unknown`；只展开对 Aim 有后果的 material lane。
5. 计算 frontier，在一轮中询问全部 Aim-relevant frontier decisions，并为每题直接给出推荐答案，然后等待用户。
6. 根据回答更新 node、增加或删除 downstream branch，并重算 frontier；Aim 改变时先显示并对齐 `Aim shift`。
7. frontier 为空，或用户明确批准、结束、给出 `Standard` 时，关闭 alignment；后两种情况把剩余判断授权给 agent 后续推断。
...
- tree 只包含 Aim-relevant decisions，frontier 中每个 prerequisite 已经 `Aligned`；
- 每个问题都有直接推荐答案，没有数值 question cap 或强制深度；
