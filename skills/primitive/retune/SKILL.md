...
- installed/global/runtime skill copies MUST 视为 projection；MUST NOT 直接把 runtime projection 当作 patch target。
- 当用户给出 runtime projection path 时，MUST 找到 owning source skill 并 patch source；找不到 source truth 时，MUST 停止并报告 blocker。
...
- When: Partita landing 中需要同步 installed/global Codex skill runtime。
  Do: MUST 运行 owning install/projection command；MUST NOT 手动编辑 installed runtime projection。
...
- Filesystem: MUST NOT 直接编辑 installed/global/runtime skill projection。
- External: none.
...
- 避免 runtime projection 被直接 patch 时，MUST 使用 [source projection case](references/source-projection-case.md)。
...
- 已定位 target skill source truth，没有直接 patch installed/global/runtime projection；
...
- 如果需要同步 installed runtime，已运行 owning install/projection command；
