...
- 一个 task phase 已完成，并且 tests、verifier、generator、CI 或用户确认已经形成闭环。
...
- MUST 从 current topic、source document、accepted conclusion、generated target、current dispatcher 或 explicit user frame 中选择 authority。
...
- 硬判断 MUST 尽量用 git status、diff、search output、generator、tests、schema、package check 或 verifier output 支撑。
...
- Filesystem: MAY read code、docs、tests、generated metadata、manifests 和 dispatcher surfaces；MAY write only approved cleanup 和 directly stale generated metadata。
- External: none.

## Workflow

1. 识别 completed phase、verification closure 和 cleanup authority；缺失时问一个最小问题。
2. 用 git status、git diff、recent commits、changed files、search、generated metadata、dispatcher 或 manifests 定位 task change surface。
