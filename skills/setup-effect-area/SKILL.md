...
description: "Use when the user asks to set up, update, migrate, verify, or repair a TypeScript/Effect target repository against effect-harness runtime and CLI contracts. Not for updating effect-harness itself, generic repo pinning, writing Effect business features, or defining Craft skill semantics."
when_to_use: "setup effect area, update Effect target, effect-harness init, Effect target setup, migrate Effect harness target, legacy harness wrapper, verify Effect harness target, setup TypeScript Effect repo, effect runtime contract"
...
Another pressure scenario: the target is already half-migrated, so the agent
patches one broken wrapper script while leaving the package baseline, verify
chain, managed runtime, and harness manifest inconsistent.
...
- remove or replace legacy target-local harness wrappers when the owning
  `effect-harness` CLI provides the replacement;
...
Root selection, update strategy, legacy-shape classification, copy ownership
review, and whether to wait for a harness self-update are `soft`; harness init
or migration commands, target verify, source status, package checks, old-wrapper
detection, and target `verify` are primitive `constraint.hard` only when
enforced by CLI commands, scripts, git, grep/package checks, or package
validation.
...
- `legacy-migration`: target has old local wrappers, source copies, or scripts
  that predate the current `effect-harness` CLI.
- `blocked-by-harness`: target cannot update until `effect-harness` exposes a
  hard setup/update/migration mechanism.
...
- Do not keep legacy target-local harness wrapper scripts in the verify path
  when the current `effect-harness` CLI owns that check.
...
5. Classify the target as `fresh-setup`, `target-update`,
   `legacy-migration`, or `blocked-by-harness`. If the required harness
   update/migration command does not exist, report that blocker instead of
   hand-writing a replacement.
6. Run setup or migration dry-run before writing:
...
- target classification is `fresh-setup`, `target-update`,
  `legacy-migration`, or `blocked-by-harness`;
- Craft skill source and `effect-harness` CLI/runtime ownership are separated;
- setup or migration dry-run happened before target writes;
