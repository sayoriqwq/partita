...
- `projection`: 把共享概念真源投影到 skill-local references，并生成 dispatcher audit。
...
当多个 skill 依赖同一概念时，generator MUST 把该概念投影到每个目标 skill 的 `references/`。
...
| `project` | Partita repo invariant | source 层失败、dispatcher drift、projection drift、broken Markdown/wiki links、迁出 surface 回流、root `SKILL.md` |
